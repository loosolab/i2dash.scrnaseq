...
plot_{{ env_id }} <- i2dash.scrnaseq::plotly_scatterplot(x = {{ env_id }}$x[,1], y = {{ env_id }}$y[,1], color = colour_by_{{ env_id }}, text = labels_{{ env_id }}, y_title = y_title_{{ env_id }}, x_title = x_title_{{ env_id }}, colors = colors_{{ env_id }}, type = "scatter") %>% plotly::toWebGL()
...
output$plot_{{ env_id }} <- plotly::renderPlotly({
  ...
  if(is.factor(colour_{{ env_id }}()$colour)){
    if(colour_{{ env_id }}()$colour_title %in% names(colormaps)) colors <- colormaps[[input$select_colour_{{ env_id }}]] else colors <- "Set1"
  } else {
    colors <- NULL
  }
