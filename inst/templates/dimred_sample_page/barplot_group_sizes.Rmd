...
sp_bars <- plotly::plot_ly(sd_{{ env_id }}, colors = colors_{{ env_id }}, x = df_{{ env_id }}[[grouping_{{ env_id }}]], color = df_{{ env_id }}[[grouping_{{ env_id }}]], type = "histogram") %>%
  plotly::layout(
...
output$plot_sp_bars_{{ env_id }} <- plotly::renderPlotly({
  ...
  sp_bars <- plotly::plot_ly(df_{{ env_id }}()$sd, colors = colors, x = df_{{ env_id }}()$df[[df_{{ env_id }}()$grouping]], color = df_{{ env_id }}()$df[[df_{{ env_id }}()$grouping]], type = "histogram") %>%
  plotly::layout(
