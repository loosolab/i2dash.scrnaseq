...
if(is.factor(df_{{ env_id }}[["{{ meta_column }}"]])){
  ...
  bars_{{ meta_column }} <- plotly::plot_ly(sd_{{ env_id }}, colors = colors_{{ env_id }}, x = df_{{ env_id }}[["{{ meta_column }}"]], color = df_{{ env_id }}[["{{ meta_column }}"]], type = "histogram") %>%
  plotly::layout(
  ...
  box_{{ meta_column }} <- plotly::plot_ly(sd_{{ env_id }}, colors = colors_{{ env_id }}, x = df_{{ env_id }}[[grouping_{{ env_id }}]], y = df_{{ env_id }}[["{{ meta_column }}"]], color = df_{{ env_id }}[[grouping_{{ env_id }}]], type = "histogram") %>%
  plotly::add_boxplot() %>%
  plotly::layout(
