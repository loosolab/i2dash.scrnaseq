...
if(is.factor(df_{{ env_id }}[["{{ meta_column }}"]])){
  ...
  bar_{{ meta_column }} <- plotly::plot_ly(sd_{{ env_id }}, colors = colors_{{ env_id }}, x = df_{{ env_id }}[["{{ meta_column }}"]], color = df_{{ env_id }}[["{{ meta_column }}"]]) %>%
  plotly::layout(
...
output$plot_box_bar_{{ meta_column }}_{{ env_id }} <- plotly::renderPlotly({
  if(is.factor(df_{{ env_id }}()$df[["{{ meta_column }}"]])){
    if("{{ meta_column }}" %in% names(colormaps)) colors <- colormaps[["{{ meta_column }}"]] else colors <- "Set1"
      bar <- plotly::plot_ly(df_{{ env_id }}()$sd, colors = colors, x = df_{{ env_id }}()$df[["{{ meta_column }}"]], color = df_{{ env_id }}()$df[["{{ meta_column }}"]]) %>%
    plotly::layout(
