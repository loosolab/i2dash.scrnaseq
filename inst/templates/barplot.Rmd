...
#' @param y_group_by A vector with values the rows of the dataframe should be grouped by.
#' @param x_group_by A vector with values the columns of the dataframe should be grouped by.
#'
...
```{r, eval=is_shiny}
ui_list <- list()
vals_{{ env_id }} <- reactiveValues(proceed = T)
#
...
reactive_{{ env_id }} <- shiny::reactive({
  ...
  # Handle modal if number of rows of the dataframe are > 100.
  if(nrow(output_list$df)>100) {
    showModal(dataModal())
    vals_{{ env_id }}$proceed <- F
  } else {
    vals_{{ env_id }}$proceed <- T
  }
  
  return(list("barplot_input_list" = output_list, "y_group_by_title" = y_group_by_title, "x_group_by_title" = x_group_by_title))
})

# Function for a modal dialog
dataModal <- function(){
  modalDialog(
    title = "Warning",
    "The barplot will have more than 100 bars.",
    easyClose = F,
    footer = tagList(modalButton("Cancel"),
                     actionButton("proceed_{{ env_id }}", "Proceed"))
  )
}

# Handle proceed button of modal
observeEvent(input$proceed_{{ env_id }},{
  vals_{{ env_id }}$proceed <- T
  removeModal()
})
...
output$plot_{{ env_id }} <- plotly::renderPlotly({
  ...
  if(vals_{{ env_id }}$proceed){
    ...
    ) %>% plotly::layout(title = "Please select another grouping")
  }
})
