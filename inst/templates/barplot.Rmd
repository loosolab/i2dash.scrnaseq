...
output_list <- create_barplot_df(y_group_by = y_group_by_{{ env_id }}[,1], x_group_by = x_group_by_{{ env_id }}[,1])

# creating the plot object
plot_{{ env_id }} <- i2dash.scrnaseq::plotly_barplot(x = output_list$x[[1]], y = output_list$y[[1]], name = output_list$names[[1]], showlegend = output_list$showlegend, x_group_by_title = x_group_by_title_{{ env_id }}, y_group_by_title = y_group_by_title_{{ env_id }})

# Provide data for download
htmltools::div(style="display:block;float:left;width:100%;height:90%;",
  htmltools::tags$button(i2dash::embed_var(output_list$df)), plot_{{ env_id }}
)
```
...
output$plot_{{ env_id }} <- plotly::renderPlotly({
  ...
  i2dash.scrnaseq::plotly_barplot(x = reactive_{{ env_id }}()$barplot_input_list$x[[1]], y = reactive_{{ env_id }}()$barplot_input_list$y[[1]], name = reactive_{{ env_id }}()$barplot_input_list$names[[1]], showlegend = reactive_{{ env_id }}()$barplot_input_list$showlegend, x_group_by_title = x_group_by_title, y_group_by_title = y_group_by_title)
})
