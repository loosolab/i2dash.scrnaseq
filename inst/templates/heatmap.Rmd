...
if(!is.null({{ env_id }}$column_split)){
  if({{ env_id }}$visualisation_mode == "splitted"){
    ...
    if(colnames({{ env_id }}$column_split)[1] %in% names(colormaps)){
      ha = ComplexHeatmap::HeatmapAnnotation(
            group_by = {{ env_id }}$column_split[,1],
            col = list(group_by = colormaps[[colnames({{ env_id }}$column_split)[1]]])
      )
    }
    ...
    if(colnames({{ env_id }}$column_split)[1] %in% names(colormaps)){
      ha = ComplexHeatmap::HeatmapAnnotation(
            group_by = colnames(exprs_values_{{ env_id }}),
            col = list(group_by = colormaps[[colnames({{ env_id }}$column_split)[1]]])
      )
    }
  }
...
# annotation in case of aggregation and splitting
# if(colnames({{ env_id }}$column_split)[1] %in% names(colormaps)){
#       ha = ComplexHeatmap::HeatmapAnnotation(
#             group_by = {{ env_id }}$column_split[,1],
#             col = list(group_by = colormaps[[colnames({{ env_id }}$column_split)[1]]])
...
output$plot_{{ env_id }} <- shiny::renderPlot({
  ...
  # add annotation along the colormap
  # ha = NULL
  # if(!is.null({{ env_id }}$column_split)){
  #   if(input$select_visualisation_{{ env_id }} %in% names(colormaps)){
  #     ha = ComplexHeatmap::HeatmapAnnotation(
  #       group_by = colnames(exprs_values_{{ env_id }}()),
  #       col = list(group_by = colormaps[[input$select_visualisation_{{ env_id }}]])
  #     )
  #   } else if(input$select_visualisation_{{ env_id }} == "samples" & input$select_column_split_{{ env_id }} != "none"){
  #     if(input$select_column_split_{{ env_id }} %in% names(colormaps)){
  #       ha = ComplexHeatmap::HeatmapAnnotation(
  #         group_by = column_split_{{ env_id }}(),
  #         col = list(group_by = colormaps[[input$select_column_split_{{ env_id }}]])
  #       )
  #     }
  #   }
  # }
