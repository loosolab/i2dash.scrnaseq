...
if(!is.null({{ env_id }}$compId) & {{ env_id }}$compId %in% edgeTable$reciever){
  ...
  df_{{ env_id }} <- shiny::reactive({
    ...
    "%ni%" <- Negate("%in%")
    additional_arguments <- env_compa$additional_arguments
    ...
    if (is.null(selection_transmitter)) {
      selection_transmitter <- all_keys_transmitter 
    } else {
      selection_transmitter
    }
    # subset dataset 
    m <- match(selection_transmitter, dt[[1]])
    subset <- dt[na.omit(m), ]
    additional_arguments$data <- subset
    
    key <- NULL
    
    return(list("params" = additional_arguments, "data" = subset, "key" = key))
  })
