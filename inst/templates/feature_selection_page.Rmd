...
output$plot_{{ env_id }} <- plotly::renderPlotly({
  ...
  i2dash.scrnaseq::plotly_scatterplot(reactive_{{ env_id }}()$complete_df, x = ~mean, y = ~total, color = ~HVGs, text = names({{ env_id }}$fit$mean), y_title = y_title, x_title = "Mean of log-expression", type = "scatter", colors = c("normal" = "#28BBD7", "HVG" = "#ED90A4")) %>%
...
```{r}
output$tbl_env_waOb0F <- DT::renderDataTable({
  options(DT.options = list(scrollY="50vh",scrollX="300px", pageLength = 100, autoWidth = TRUE))
  DT::datatable(reactive_{{ env_id }}()$hvg_df, filter = 'top')
})
DT::dataTableOutput('tbl_env_waOb0F')
