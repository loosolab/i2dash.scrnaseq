### Table shows features and the number of samples with expression > 0/ total number of samples grouped by metadata:
...
```{r, eval=!is_shiny}
i2dash.scrnaseq::plotly_violinplot(y = exprs_values_{{ env_id }}[1,], group_by = group_by_{{ env_id }}[,1], y_title = "Expression", group_by_title = colnames(group_by_{{ env_id }})[1]) %>%
  plotly::layout(
