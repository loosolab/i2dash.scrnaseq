...
if(!is.null({{ env_id }}$group_by_title)) group_by_title_{{ env_id }} <- {{ env_id }}$group_by_title else group_by_title_{{ env_id }} <- colnames({{ env_id }}$group_by)[1]

plot_{{ env_id }} <- i2dash.scrnaseq::plotly_violinplot(y = {{ env_id }}$y[,1], group_by = {{ env_id }}$group_by[,1], y_title = y_title_{{ env_id }}, group_by_title = group_by_title_{{ env_id }})

# Provide data for download
htmltools::div(style="display:block;float:left;width:100%;height:90%;",
  htmltools::tags$button(i2dash::embed_var(data.frame({{ env_id }}$y[,1], {{ env_id }}$group_by[,1]))), plot_{{ env_id }}
)
```
