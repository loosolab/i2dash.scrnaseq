...
create_df <- function(reduced_dim, metadata, grouping, count_table){
...
```{r, eval=!is_shiny}
df_{{ env_id }} <- create_df(reduced_dim, metadata, grouping, count_table)
...
df_{{ env_id }} <- shiny::reactive({
  ...
  df <- create_df(reduced_dim = reduced_dim, metadata = metadata, grouping = grouping, count_table = count_table)
