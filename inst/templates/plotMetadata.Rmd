...
if(!is.null({{ env_id }}$group_by)){
  if(is.numeric(SummarizedExperiment::colData({{ env_id }}$object)[{{ env_id }}$y][[1]]) | is.numeric(SummarizedExperiment::colData({{ env_id }}$object)[{{ env_id }}$x][[1]])){
...
if(!is.null({{ env_id }}$metadata[1])){
  if({{ env_id }}$metadata[1] %in% names(colormaps)) plot_{{ env_id }} <- plot_{{ env_id }} + ggplot2::scale_fill_manual(values=colormaps[[{{ env_id }}$metadata[1]]])
}
# convert into a plotly object if as_plotly = TRUE
if({{ env_id }}$as_plotly){
...
reactive_{{ env_id }} <- shiny::reactive({
  ...
  if(!is.null(group_by)){
    if(is.numeric(SummarizedExperiment::colData({{ env_id }}$object)[[y]]) | is.numeric(SummarizedExperiment::colData({{ env_id }}$object)[[x]])){
      if(is.factor(SummarizedExperiment::colData({{ env_id }}$object)[[group_by]])){
  ...
  if(!is.null(colour_by)){
    if(colour_by %in% names(colormaps)) plot <- plot + ggplot2::scale_fill_manual(values=colormaps[[colour_by]])
  }
