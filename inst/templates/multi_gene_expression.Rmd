...
if({{ env_id }}$selection_reduced_dim){
  selectInput("select_dimred_{{ env_id }}", label = "Select dimension reduction:", choices = names(reduced_dim_{{ env_id }}))
}
# selection field for gene
selectInput("select_gene_{{ env_id }}", label = "Select genes:", choices = rownames(count_table_{{ env_id }}), multiple = TRUE)
...
reactive_{{ env_id }} <- eventReactive(input$generate_{{ env_id }}, {
  ...
  if({{ env_id }}$selection_reduced_dim){
    red_dim <- reduced_dim_{{ env_id }}[[input$select_dimred_{{ env_id }}]]
  } else {
    red_dim <- reduced_dim_{{ env_id }}[[1]]
  }
  gene_list <- input$select_gene_{{ env_id }}
  df <- data.frame(x = red_dim[,1], y = red_dim[,2], t(count_table_{{ env_id }}))
