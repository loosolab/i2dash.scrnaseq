...
# # handle colormaps
if("colour_by" %in% names({{ env_id }}$params)){
  if(is.factor(SummarizedExperiment::colData({{ env_id }}$object)[[{{ env_id }}$params$colour_by]]) | is.logical(SummarizedExperiment::colData({{ env_id }}$object)[[{{ env_id }}$params$colour_by]])){
    if({{ env_id }}$params$colour_by %in% names(colormaps)) plot <- plot + ggplot2::scale_fill_manual(values=colormaps[[{{ env_id }}$params$colour_by]])
  }
}
...
reactive_plot_{{ env_id }} <- shiny::eventReactive(input$generate_{{ env_id }},{
  ...
  # handle colormaps
  if(!is.null(reactive_{{ env_id }}()$colour_by)){
    if(is.factor(SummarizedExperiment::colData({{ env_id }}$object)[[reactive_{{ env_id }}()$colour_by]]) | is.logical(SummarizedExperiment::colData({{ env_id }}$object)[[reactive_{{ env_id }}()$colour_by]])){
      if(reactive_{{ env_id }}()$colour_by %in% names(colormaps)) plot <- plot + ggplot2::scale_fill_manual(values=colormaps[[reactive_{{ env_id }}()$colour_by]])
    }
  }
