...
#' @param highlight A character string identical to one of the column names in 'metadata'.
#' @param title The title of the page.
#' @param labels (Optional) A vector with labels. Should be of the same length as the rownumber of 'reduced_dim'.
#' @param barplot_highlight (Default: TRUE) A barplot with the number of observations in 'highlight' will be created.
...
#' @export
add_dim_reduction_page <- function(object, reduced_dim, metadata, highlight, title, labels = NULL, barplot_highlight = TRUE, menu = NULL, sidebar = NULL) {
  ...
  if(!highlight %in% colnames(metadata)) stop("'metadata' does not contain a column with this name.")
  if(is.factor(metadata$highlight)) stop("'highlight' in 'metadata' should contain factors and be of class 'factor'.")
  if(!is.character(title)) stop("'title' should be a character vector.")
  ...
  env$labels <- labels
  #env$barplot_highlight <- barplot_highlight
  ...
  timestamp <- Sys.time()
  #dim_reduction <- knitr::knit_expand(file = system.file("templates", "boxplot.Rmd", package = "i2dash.scrnaseq"), title = title, env_id = env_id, date = timestamp)
  ...
  max_comp <- max_comp + 1
  if(barplot_highlight){
    barplot_highlight_component <- knitr::knit_expand(file = system.file("templates", "red_dim_2.Rmd", package = "i2dash.scrnaseq"), env_id = env_id, date = timestamp)
    expanded_components <- append(expanded_components, barplot_highlight_component)
    max_comp <- max_comp + 1
  }

  highlight_index <- which(colnames(metadata) == highlight)
  meta_names <- colnames(metadata)[-highlight_index]
  for(i in meta_names){
    if(max_comp < 4){
