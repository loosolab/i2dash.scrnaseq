...
setMethod("verbatim_text",
          ...
          function(dashboard, transmitter = NULL, event = "plotly_selected", title = NULL) {
            ...
            # Expand component
            timestamp <- Sys.time()
            expanded_component <- knitr::knit_expand(file = system.file("templates", "verbatim_text.Rmd", package = "i2dash.scrnaseq"), title = title, env_id = env_id, date = timestamp)
