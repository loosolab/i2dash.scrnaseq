...
#' @export
setMethod("add_umap_comparison_page",
          signature = signature(dashboard = "i2dashboard", object = "missing"),
          function(dashboard, exprs_values, seed = NULL, page = "umap_comparison_page", title = "Explore UMAP parameter", menu = "Tools", ...){
            ...
            scater.calculateUMAP <- list(x = exprs_values, ...)

            # Create component environment
            env <- new.env()
            env$scater.calculateUMAP <- scater.calculateUMAP
            env$seed <- seed
            saveRDS(env, file = file.path(dashboard@datadir, paste0(env_id, ".rds")))

            # Expand component
            timestamp <- Sys.time()
            umap_comparison_component <- knitr::knit_expand(file = system.file("templates", "umap_comparison_page.Rmd", package = "i2dash.scrnaseq"), env_id = env_id, date = timestamp)
...
setMethod("add_umap_comparison_page",
          ...
          function(dashboard, object, exprs_values = "logcounts", subset_row = NULL, ...){
            ...
            add_umap_comparison_page(dashboard, exprs_values = exprs_values, ...)
          })
